# Roadmap: Memoon

**Project:** Memoon — Content Structure Manager
**Created:** 2025-03-11

---

## Phase 1: Foundation

**Goal:** Ensure the core application is stable and usable

**Requirements:** 

- [ ] **CORE-01**: Application runs without errors
- [ ] **CORE-02**: All CRUD operations work
- [ ] **CORE-03**: CSV import functions correctly

**Success Criteria:**
1. Server starts without errors
2. Can create, read, update, delete all models
3. CSV import successfully creates shots

---

## Phase 2: Enhanced Features

**Goal:** Add features to improve usability

**Requirements:**

- [ ] **FEAT-01**: Search functionality across all models
- [ ] **FEAT-02**: Better filtering and sorting
- [ ] **FEAT-03**: Export to common formats (JSON, CSV)

**Success Criteria:**
1. Search returns relevant results
2. Filters work on list views
3. Export produces valid files

---

## Phase 3: Polish

**Goal:** Improve user experience and fix issues

**Requirements:**

- [ ] **POLI-01**: Fix any bugs discovered
- [ ] **POLI-02**: Improve UI/UX
- [ ] **POLI-03**: Add user documentation

**Success Criteria:**
1. No critical bugs
2. UI is intuitive
3. README is complete

---

## Phase 4: Add dark mode

**Goal:** Add dark mode with theme toggle and persistence

**Requirements**: TBD

**Depends on:** Phase 3

**Plans:** 1 plan

- [x] 04-01-PLAN.md — Add dark mode with toggle and persistence

---

## Traceability

| Requirement | Phase | Status |
|-------------|-------|--------|
| CORE-01 | Phase 1 | Complete |
| CORE-02 | Phase 1 | Complete |
| CORE-03 | Phase 1 | Complete |
| FEAT-01 | Phase 2 | Pending |
| FEAT-02 | Phase 2 | Pending |
| FEAT-03 | Phase 2 | Pending |
| POLI-01 | Phase 3 | Pending |
| POLI-02 | Phase 3 | Pending |
| POLI-03 | Phase 3 | Pending |

### Phase 5: Add props model

**Goal:** [To be planned]
**Requirements**: TBD
**Depends on:** Phase 4
**Plans:** 0 plans

Plans:
- [ ] TBD (run /gsd:plan-phase 5 to break down)

### Phase 6: Add user roles and permissions

**Goal:** [To be planned]
**Requirements**: TBD
**Depends on:** Phase 5
**Plans:** 0 plans

Plans:
- [ ] TBD (run /gsd:plan-phase 6 to break down)

---

*Roadmap created: 2025-03-11*
